...
const generator_3 = require("../generator/copilotPlugin/generator");
var TemplateNames;
...
const Feature2TemplateName = {
    ...
    [`${create_1.CapabilityOptions.m365SearchMe().id}:undefined`]: TemplateNames.M365MessageExtension,
...
class Coordinator {
    async create(context, inputs, actionContext) {
        ...
        if (scratch === create_1.ScratchOptions.no().id) {
            ...
            else if (inputs[questionNames_1.QuestionNames.Capabilities] === create_1.CapabilityOptions.copilotPluginApiSpec().id ||
                inputs[questionNames_1.QuestionNames.Capabilities] === create_1.CapabilityOptions.copilotPluginOpenAIPlugin().id) {
                const res = await generator_3.CopilotPluginGenerator.generate(context, inputs, projectPath);
                if (res.isErr()) {
                    return teamsfx_api_1.err(res.error);
                }
            }
    ...
    async provision(ctx, inputs, actionContext) {
        ...
        if (containsM365) {
            ...
            const checkM365TenatRes = await provisionUtils_1.provisionUtils.ensureM365TenantMatchesV3(tenantSwitchCheckActions, m365tenantInfo === null || m365tenantInfo === void 0 ? void 0 : m365tenantInfo.tenantIdInToken);
                    ...
                    if (ctx.platform !== teamsfx_api_1.Platform.CLI) {
                        (_c = ctx.ui) === null || _c === void 0 ? void 0 : _c.showMessage("info", msg, false, adminPortal).then((value) => {
                            if (value.isOk() && value.value === adminPortal) {
                                ctx.ui.openUrl(constants_2.Constants.TEAMS_ADMIN_PORTAL);
                            }
                        });
                    }
                    else {
