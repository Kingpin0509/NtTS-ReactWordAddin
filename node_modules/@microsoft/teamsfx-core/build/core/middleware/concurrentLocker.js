...
const ConcurrentLockerMW = async (ctx, next) => {
        ...
        try {
            ...
            try {
                ...
                if (retryNum > 0) {
                    ...
                    telemetry_1.sendTelemetryErrorEvent(error_1.CoreSource, "concurrent-operation", new common_1.ConcurrentError(error_1.CoreSource), { retry: retryNum + "", acquired: "true", doing: doingTask, todo: taskName });
                }
                await next();
            }
            finally {
    ...
    if (!acquired) {
        ...
        telemetry_1.sendTelemetryErrorEvent(error_1.CoreSource, "concurrent-operation", new common_1.ConcurrentError(error_1.CoreSource), {
            retry: retryNum + "",
            acquired: "false",
            doing: doingTask || "",
            todo: taskName,
        });
        ctx.result = teamsfx_api_1.err(new common_1.ConcurrentError(error_1.CoreSource));
    }
};
