        ...
        children: [
            {
                ...
                children: [
                    {
                        condition: (inputs) => inputs.projectPath !== undefined &&
...
function selectAadAppManifestQuestionNode() {
    return {
        ...
        children: [
            {
                condition: (inputs) => inputs.projectPath &&
...
function confirmCondition(inputs, isLocal) {
    return (inputs.platform !== teamsfx_api_1.Platform.CLI_HELP &&
        inputs.platform !== teamsfx_api_1.Platform.CLI &&
        inputs.platform !== teamsfx_api_1.Platform.VS &&
        inputs.projectPath &&
        inputs[questionNames_1.QuestionNames.TeamsAppManifestFilePath] &&
        path.resolve(inputs[questionNames_1.QuestionNames.TeamsAppManifestFilePath]) !==
...
function selectTeamsAppManifestQuestion(isLocal = false) {
    return {
        name: isLocal
...
function selectTeamsAppValidationMethodQuestionNode() {
    return {
        data: selectTeamsAppValidationMethodQuestion(),
    };
}
exports.selectTeamsAppValidationMethodQuestionNode = selectTeamsAppValidationMethodQuestionNode;
...
function selectTargetEnvQuestion(questionName = questionNames_1.QuestionNames.TargetEnvName, remoteOnly = true, throwErrorIfNoEnv = false, defaultValueIfNoEnv = environment_1.environmentManager.getDefaultEnvName()) {
    return {
        type: "singleSelect",
        name: questionName,
...
function selectAadManifestQuestion() {
    return {
        ...
        title: localizeUtils_1.getLocalizedString("core.selectAadManifestQuestion.title"),
        type: "singleFile",
        default: (inputs) => {
            if (!inputs.projectPath)
                return undefined;
...
async function envQuestionCondition(inputs) {
    ...
    const requireAad = appType.includes(collaborator_1.CollaborationConstants.AadAppQuestionId);
    const requireTeams = appType.includes(collaborator_1.CollaborationConstants.TeamsAppQuestionId);
